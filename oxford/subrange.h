...
//
//  Copyright Â© 2005, 2012, 2014  Russel Winder
//
 ...
 *
 *  <p>Copyright Â© 2005, 2012, 2014  Russel Winder.</p>
 *
...
public:
    ...
    {
        if (((proposed + 1) < (RangeTraits::min() + 1)) || (proposed > RangeTraits::max()))
        {
            throw typename RangeTraits::exception_type();
        }
        return proposed;
    }
};
...
public:
    ...
    {
        while ((proposed + 1) < (RangeTraits::min() + 1) )
        {
            proposed += RangeTraits::max() - RangeTraits::min();
        }
        while (proposed > RangeTraits::max())
        {
            proposed -= RangeTraits::max() - RangeTraits::min();
        }
        return proposed;
...
{
public:
    static typename RangeTraits::value_type plusNaN()
    {
        return std::numeric_limits<typename RangeTraits::value_type>::max();
    }
    static typename RangeTraits::value_type minusNaN()
    {
        return std::numeric_limits<typename RangeTraits::value_type>::min();
    }
    static bool isNaN(typename RangeTraits::value_type const v)
    {
        return v == plusNaN() || v == minusNaN();
    }
    static typename RangeTraits::value_type checkValue(typename RangeTraits::value_type const proposed)
    {
        return
...
public :
    ...
    static value_type min()
    {
        return RangeTraits::min();
    }
    static value_type max()
    {
        return RangeTraits::max();
    }
private :
    value_type value;
public:
    subrange() : value(min()) { }
    explicit subrange(value_type const v)
    {
        value = ArithmeticPolicy<RangeTraits>::checkValue(v);
    }
    operator value_type() const
    {
        return value;
    }
    subrange & operator=(value_type const v)
    {
        value = ArithmeticPolicy<RangeTraits>::checkValue(v);
        return *this;
    }
...
public:
    ...
    subrange & operator+=(value_type const v)
    {
        value = ArithmeticPolicy<RangeTraits>::checkValue(value + v);
        return *this;
    }
    subrange & operator-=(value_type const v)
    {
